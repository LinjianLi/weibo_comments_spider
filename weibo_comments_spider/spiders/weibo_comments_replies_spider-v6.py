...
class WeiboCommentsRepliesSpider(Spider):
    """
    这一个版本的爬虫无法很好处理爬取用户信息时出现 418 的情况
    因为微博对于用户信息页面的反爬力度大很多
    而几个 Request 是串联在一起的，所以爬用户信息遇到 418 的时候，
    前面爬下来的评论、回复都无法顺利存入数据库了
    """
    name = "weibo_comments_replies_spider-v6"
    download_delay = 2
            ...
            if 'data' in result.keys():
                for single_comment in result['data']:
                    ...
                    if single_comment['more_info_type'] == 1:
                        ...
                        replies = requests.get(self.folded_replies_url.format(cid=s_c_id)).text
                    ...
                    meta = {'item': item,
                            'item_entry': 'commenter',
                            'replier_id': replier_id}
                    try:
                        yield Request(url=self.user_info_url.
                                      format(user_id=commenter_id),
                                      headers=self.fake_headers,
                                      callback=self.parse_user_info,
                                      meta=meta)
                    except:
                        item['commenter'] = '<user_id: '+commenter_id+'user_info: anti-crawled>'
                        item['replier'] = '<user_id: '+replier_id+'user_info: anti-crawled>'
                        yield item
    ...
    def parse_user_info(self, response):
        ...
        results = json.loads(response_text)
        if results['ok'] == 0:
            raise Exception("In function [parse_user_info], results['ok'] == 0")
        if 'data' in results.keys():
            result = results['data']
            ...
            item[meta['item_entry']] = user_info
            if ('replier_id' in meta) and (meta['replier_id'] != ''):
                replier_id = meta['replier_id']
                meta = {'item': item,
                        'item_entry': 'replier'}
                try:
                    yield Request(url=self.user_info_url.
                                  format(user_id=replier_id),
                                  headers=self.fake_headers,
                                  callback=self.parse_user_info,
                                  meta=meta)
                except:
